...
int main(int argc, char *argv[])
{
	...
	input1 = (float *)malloc(sizeof(float)*blocksize); //should be blocksize long
	...
	while(1){
		...
		for( j = 0; j < 8; j++){
			...
			arm_fir_decimate_f32(s1, input2, input_decimated_2, blocksize);
			arm_fir_decimate_f32(s, input1, input_decimated_1, blocksize);
		...
		peak_index = fft(buffer, 30, button_flag);
